...
export type DeckMapView = 'global';
    ...
    if (useTechHQ && this.techHQSC) {
      this.techHQClusters = this.techHQSC.getClusters(bbox, zoom).map(f => {
        ...
        return {
          ...
          primaryType: (item.type ?? 'public') as 'faang' | 'unicorn' | 'public',
  ...
  private createControls(): void {
    ...
    controls.innerHTML = `
      ...
      <div class="view-selector">
        <span class="view-label" style="color:var(--text-secondary);font-size:11px;padding:4px 8px;">Türkiye</span>
      </div>
    `;
    ...
    // View locked to Turkey — no region selector needed
  }
