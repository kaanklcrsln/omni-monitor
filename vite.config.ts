...
const VARIANT_META: Record<string, {
  ...
  turkey: {
    title: 'Türkiye Monitor - GIS & Durum İzleme',
    description: 'Türkiye odaklı gerçek zamanlı GIS izleme paneli. Deprem, yangın, haber, piyasa, enerji altyapısı ve iklim verileri tek ekranda.',
    keywords: 'Türkiye, GIS, deprem, yangın, AFAD, BIST, harita, izleme, durum farkındalığı, enerji, boru hattı, liman, haber, piyasa, iklim',
    url: '',
    siteName: 'Türkiye Monitor',
    shortName: 'TurkiyeMonitor',
    subject: 'Türkiye GIS ve Durum İzleme',
    classification: 'GIS Dashboard, İzleme Paneli, Haber Toplayıcı',
    categories: ['news', 'productivity'],
    features: [
      'Deprem izleme (USGS)',
      'Uydu yangın tespiti (NASA FIRMS)',
      'Türkiye haberleri',
      'BIST piyasa verileri',
      'Enerji boru hatları',
      'Liman izleme',
      'İklim anomalileri',
      'Boğaz trafiği',
...
const activeVariant = 'turkey';
const activeMeta = VARIANT_META.turkey;
...
export default defineConfig({
  ...
  preview: {
    ...
    allowedHosts: true,
    headers: {
      'Content-Security-Policy': "default-src 'self'; connect-src 'self' https: ws: wss: blob: data:; img-src 'self' data: blob: https:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval'; worker-src 'self' blob:; font-src 'self' data: https:; media-src 'self' data: blob: https:; frame-src 'self' https://www.youtube.com https://www.youtube-nocookie.com https://s3.tradingview.com;",
    },
  },
  server: {
    port: 3000,
